...
## 開発サイクル方針（2026-01-13決定）

### 方針：4メニュー完了後にcc-sdd実施

**決定事項**: 営業活動4メニュー（リード管理、架電管理、商談管理、成約管理）のUIベース改善を**すべて完了してから**、cc-sdd手続き（企画書・要件定義反映、承認フロー、テスト実施）に移行する。

### 根拠

| 判断基準 | 状況 | 評価 |
|----------|------|:----:|
| 部分リリースの必要性 | なし（架電管理単独リリース予定なし） | ✅ |
| 品質クリティカル | 重大な問題なし | ✅ |
| 残メニューの規模 | 商談・成約管理は架電管理ほど大きくない | ✅ |

### 理論的背景（UIベースSDD開発手法 5.11.2より）

```
UIプロトタイプ完成度フェーズ1（ラフUI段階）では、
「動くもの優先」で4メニューすべてを回し、
フェーズ2（UI起点の改善）終了後にcc-sdd統合が効率的
```
...
- [ ] 変更時のGitチェックポイント作成
- [ ] 主要変更時のPROJECT_CONTROL.md更新
- [ ] 重大バグ発見時の即時記録（インシデント記録）

### 予定タイムライン

| フェーズ | 内容 | 状態 |
|----------|------|:----:|
| Phase 1-1 | リード管理UI改善 | ✅ ほぼ完了 |
| Phase 1-2 | 架電管理UI改善 | ✅ ほぼ完了（多くの改修実施済み） |
| Phase 1-3 | 商談管理UI改善 | ⏳ 次に着手 |
| Phase 1-4 | 成約管理UI改善 | - |
| Phase 2 | cc-sdd実施（企画書/要件定義反映、承認、テスト） | - |
...
| 日付 | 更新内容 | 更新者 |
|------|---------|--------|
| 2026-01-13 | 開発サイクル方針追加（4メニュー完了後にcc-sdd） | - |
