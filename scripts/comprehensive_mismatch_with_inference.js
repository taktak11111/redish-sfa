// CSVファイルとデータベースの不整合を網羅的にリストアップ（推測ロジック含む）
const fs = require('fs');
...
function inferCallData(csvData) {
  ...
  if (csvData.lastCalledDate) {
    ...
    inferred.reasoning.push(`直近架電日(${csvData.lastCalledDate})が存在`);
  }
  
  // 2. ISステータスが存在する → 架電している
  ...
  if (csvData.reasonForUnavailability) {
    ...
    inferred.reasoning.push(`対応不可/失注理由(${csvData.reasonForUnavailability})が存在 → 通電して失注`);
  }
  
  // 5. 担当ISが存在する → 架電している可能性が高い（OMC/TEMPOSの場合）
  if (csvData.staffIs) {
...
function main() {
  ...
  // OC0280のデータ: W列=担当IS, X列=ISステータス, Y列=ステータス更新日, Z列=対応不可/失注理由
  //                 AB列=結果/コンタクト状況, AC列=直近架電日, AD列=架電数カウント
  // ただし、スプレッドシートが正しいデータソースのため、画像の列マッピングも考慮
  const colIndex = {
    leadId: 1,        // B: リードID
    leadSource: 2,    // C: リードソース
    linkedDate: 3,    // D: 連携日
    staffIs: 22,      // W: 担当IS（OC0280のデータから確認）
    statusIs: 23,     // X: ISステータス（OC0280のデータから確認）
    statusUpdateDate: 24, // Y: ステータス更新日（OC0280のデータから確認）
    reasonForUnavailability: 25, // Z: 対応不可/失注理由（OC0280のデータから確認）
    resultContactStatus: 27, // AB: 結果/コンタクト状況（OC0280のデータから確認: インデックス27="不通"）
    lastCalledDate: 28, // AC: 直近架電日（OC0280のデータから確認: インデックス28="2026/01/06"）
    callCount: 29,    // AD: 架電数カウント（OC0280のデータから確認: インデックス29="5"）
    conversationMemo: 31, // AF: 会話メモ・その他（TM3606のデータから確認: インデックス31="運転中で会話できず"）
  };
  ...
  mismatches.omcNotCalled.slice(0, 10).forEach(m => {
    console.log(`  ${m.leadId}: CSV[status_is=${m.statusIs}, call_count=${m.callCount !== null ? m.callCount : '(空)'}, result=${m.resultContactStatus || '(空)'}]`);
  });
  if (mismatches.omcNotCalled.length > 10) {
    console.log(`  ... 他${mismatches.omcNotCalled.length - 10}件`);
  }
  
  console.log(`\n【カテゴリ2】OMCリードで推測が必要なレコード: ${mismatches.omcNeedsInference.length}件`);
  mismatches.omcNeedsInference.slice(0, 10).forEach(m => {
    console.log(`  ${m.leadId}:`);
    console.log(`    推測: 架電している (${m.inferred.reasoning.join(', ')})`);
    console.log(`    推測call_count: ${m.inferred.callCount || '(推測不可)'}`);
    console.log(`    推測result: ${m.inferred.resultContactStatus || '(推測不可)'}`);
  });
  if (mismatches.omcNeedsInference.length > 10) {
    console.log(`  ... 他${mismatches.omcNeedsInference.length - 10}件`);
  }
  
  console.log(`\n【カテゴリ3】TEMPOSリードでCSVに明示的なデータがあるのにDBが未架電（直近7日以外）: ${mismatches.temposNotCalled.length}件`);
  mismatches.temposNotCalled.slice(0, 10).forEach(m => {
    console.log(`  ${m.leadId}: CSV[status_is=${m.statusIs}, call_count=${m.callCount !== null ? m.callCount : '(空)'}, result=${m.resultContactStatus || '(空)'}]`);
  });
  if (mismatches.temposNotCalled.length > 10) {
    console.log(`  ... 他${mismatches.temposNotCalled.length - 10}件`);
  }
  
  console.log(`\n【カテゴリ4】TEMPOSリードで推測が必要なレコード（直近7日以外）: ${mismatches.temposNeedsInference.length}件`);
  mismatches.temposNeedsInference.slice(0, 10).forEach(m => {
    console.log(`  ${m.leadId}:`);
  ...
  if (mismatches.temposNeedsInference.length > 10) {
    console.log(`  ... 他${mismatches.temposNeedsInference.length - 10}件`);
  }
