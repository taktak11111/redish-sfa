...
### 8. データマイグレーション関数の設計（INCIDENT-001）

#### 発生した問題（2026-01-11）
- 設定画面で「失注サブ理由（競合要因）」を5件に変更→保存しても、画面リロード後に6件に戻る
- `applyDropdownSettingsMigrations`関数が「6件未満なら補完」という緩すぎる条件で、ユーザーの意図的な編集を上書きしていた
- **影響時間**: 約30分（検証・修正の繰り返し）

#### 根本原因
マイグレーション関数の発動条件が「移行対象の特定」ではなく「件数比較」になっていた。
...
if (current.length < desired.length) {
  // 補完 → ユーザーの意図的な削除も上書きされる
}

// ✅ 良い例：旧値の存在で判断（明示的な移行対象）
const hasLegacyValues = current.some(o => o.value === '旧値A' || o.value === '旧値B')
if (hasLegacyValues) {
  // 旧値を含む場合のみ変換・補完
}
```
...
3. **画面をリロードする（F5）**
4. 変更が維持されているか確認
5. **別の画面（架電詳細等）に移動して、ドロップダウンに反映されているか確認**

**✅ チェックリスト**
- [ ] マイグレーション関数の発動条件は「旧値の存在」で判断しているか
- [ ] 「〇件未満なら補完」のような緩い条件を使用していないか
- [ ] 設定変更後、**リロード→反映確認**までテストしたか
- [ ] マイグレーション後のユーザー編集は上書きされないか確認したか
- [ ] ログを追加して、マイグレーションが意図せず発動していないか確認できるようにしたか
