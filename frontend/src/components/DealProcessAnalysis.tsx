...
export function DealProcessAnalysis() {
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">商談分析</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card p-5 bg-white border-l-4 border-blue-500">
          ...
          <p className="text-2xl font-bold text-blue-600 mt-1">{isLoading ? '---' : `${data?.processMetrics.winRate.toFixed(1)}%`}</p>
        ...
        <div className="card p-5 bg-white border-l-4 border-indigo-500">
          ...
          <p className="text-2xl font-bold text-indigo-600 mt-1">{isLoading ? '---' : `${data?.processMetrics.avgDealTime.toFixed(0)}日`}</p>
        ...
        <div className="card p-5 bg-white border-l-4 border-green-500">
          ...
          <p className="text-2xl font-bold text-green-600 mt-1">{isLoading ? '---' : data?.funnel[1]?.count.toLocaleString() || 0}</p>
        ...
        <div className="mt-4 grid grid-cols-3 gap-4">
          {data?.funnel.map((stage) => (
            <div key={stage.stage} className="text-center">
              ...
              <p className="text-xs text-gray-500 mt-1">転換率: {stage.conversion.toFixed(1)}%</p>
            </div>
          ))}
        </div>
      </div>
