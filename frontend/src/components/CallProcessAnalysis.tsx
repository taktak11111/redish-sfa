...
export function CallProcessAnalysis() {
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-end flex-wrap gap-4">
        ...
        <DateRangeFilter defaultPreset="thisMonth" onChange={setDateRange} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card p-5 bg-white border-l-4 border-blue-500">
          ...
          <p className="text-2xl font-bold text-blue-600 mt-1">
            {isLoading
              ? '---'
              : data?.processMetrics?.avgCallsToAppointment && data.processMetrics.avgCallsToAppointment > 0
                ? `${((1 / data.processMetrics.avgCallsToAppointment) * 100).toFixed(1)}%`
                : '0%'}
          </p>
          <p className="text-xs text-gray-400 mt-2">
            平均{' '}
            {isLoading ? '---' : data?.processMetrics?.avgCallsToAppointment ? data.processMetrics.avgCallsToAppointment.toFixed(1) : '---'}{' '}
            架電で1アポ獲得
          </p>
        </div>
        <div className="card p-5 bg-white border-l-4 border-indigo-500">
          ...
          <p className="text-2xl font-bold text-indigo-600 mt-1">
            {isLoading
              ? '---'
              : data?.processMetrics?.avgAppointmentToSetup && data.processMetrics.avgAppointmentToSetup > 0
                ? `${((1 / data.processMetrics.avgAppointmentToSetup) * 100).toFixed(1)}%`
                : '0%'}
          </p>
          <p className="text-xs text-gray-400 mt-2">
            平均{' '}
            {isLoading
              ? '---'
              : data?.processMetrics?.avgAppointmentToSetup
                ? data.processMetrics.avgAppointmentToSetup.toFixed(1)
                : '---'}{' '}
            アポで1商談設定
          </p>
        </div>
        <div className="card p-5 bg-white border-l-4 border-green-500">
          ...
          <p className="text-2xl font-bold text-green-600 mt-1">
            {isLoading
              ? '---'
              : data?.funnel && data.funnel.length > 0 && data.funnel[0].count > 0
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data?.funnel} layout="vertical">
                ...
                <YAxis dataKey="stage" type="category" width={100} axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#9ca3af' }} />
                <Tooltip
        ...
        <div className="mt-4 grid grid-cols-3 gap-4">
          {data?.funnel.map((stage, index) => (
            <div key={stage.stage} className="text-center">
              ...
              {index > 0 && <p className="text-xs text-gray-500 mt-1">転換率: {stage.conversion.toFixed(1)}%</p>}
            </div>
          ))}
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                [...Array(2)].map((_, i) => (
                  <tr key={i}>
                    <td colSpan={4} className="px-6 py-4 animate-pulse">
                ...
                data?.stageDetails.map((stage) => (
                  <tr key={stage.stage} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">{stage.successRate.toFixed(1)}%</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">{stage.successRate.toFixed(1)}%</td>
