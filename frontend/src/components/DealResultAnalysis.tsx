...
export function DealResultAnalysis() {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end flex-wrap gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">商談の成果（成約/失注/進行）を期間で可視化</p>
        </div>
                ...
                <div className="flex-1 bg-gray-200 rounded-full h-6 overflow-hidden">
                  <div
                    className={`h-full rounded-full ${
                      rank === 'A:80%' ? 'bg-green-500' : rank === 'B:50%' ? 'bg-blue-500' : rank === 'C:20%' ? 'bg-yellow-500' : 'bg-red-500'
                    }`}
              ...
              <tbody>
                {channelStats.map((channel) => (
                  <tr key={channel.name} className="border-b last:border-0">
                    ...
                    <td className="py-3 text-sm text-right text-gray-600">{channel.deals > 0 ? Math.round((channel.contracts / channel.deals) * 100) : 0}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
