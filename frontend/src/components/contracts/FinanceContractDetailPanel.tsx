...
// 融資契約情報
interface FinanceContractFormData {
  // 1. 契約情報
  contractDate?: string
  ...
  storeName?: string
  // 4. 融資サービス費用
  initialFee?: number // 初期費用（基本: ¥30,000）
  loanAmount?: number // 融資支援金額
  successFeeRate?: number // 成功報酬率（基本: 3.5%）
...
interface FinanceContractDetailPanelProps {
  contract: Deal
  onClose: () => void
...
export function FinanceContractDetailPanel({ 
  contract, 
  onClose, 
  onSave, 
  isSaving,
  ...
  const [contractFormData, setContractFormData] = useState<FinanceContractFormData>({})
  ...
  const handleContractFormChange = (field: keyof FinanceContractFormData, value: string | number | boolean | undefined) => {
    setContractFormData(prev => {
      ...
      // 融資金額または成功報酬率が変更されたら成功報酬を再計算
      if (field === 'loanAmount' || field === 'successFeeRate') {
        const loanAmount = field === 'loanAmount' ? (value as number) : (prev.loanAmount || 0)
        const rate = field === 'successFeeRate' ? (value as number) : (prev.successFeeRate || 3.5)
        updated.successFee = Math.round(loanAmount * rate / 100)
      }
      return updated
    })
  }
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold text-gray-900">
              成約詳細（融資）
              <span className="ml-2 px-2 py-1 text-xs font-semibold bg-purple-100 text-purple-700 rounded">融資</span>
            </h2>
            ...
            {activeTab === 'contract' && (
              <>
                ...
                <section className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                  <h3 className="text-sm font-semibold text-purple-700 mb-3">成約基本情報（商談から継承）</h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                    <div><span className="text-gray-500">商談ID:</span> <span className="font-medium">{dealId}</span></div>
                    <div><span className="text-gray-500">サービス:</span> <span className="font-medium text-purple-600">{formData.service || '融資'}</span></div>
                  ...
                  {expandedSections.contractInfo && (
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input type="text" value={contractFormData.contractId || ''} onChange={(e) => handleContractFormChange('contractId', e.target.value)} className={editableInputClass} placeholder="FN0001" />
                      </div>
                    </div>
                  )}
                </section>
                ...
                <section>
                  ...
                  {expandedSections.serviceFee && (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="label">初期費用 <span className="text-xs text-gray-400">（基本: ¥30,000）</span></label>
                        <input type="number" value={contractFormData.initialFee ?? 30000} onChange={(e) => handleContractFormChange('initialFee', Number(e.target.value))} className={editableInputClass} />
                      </div>
                      <div>
                        <label className="label">融資支援金額</label>
                        <input type="number" value={contractFormData.loanAmount || ''} onChange={(e) => handleContractFormChange('loanAmount', Number(e.target.value))} className={editableInputClass} placeholder="¥" />
                      </div>
                      <div>
                        <label className="label">成功報酬率 <span className="text-xs text-gray-400">（基本: 3.5%）</span></label>
                        <div className="flex items-center gap-2">
                          <input type="number" step="0.1" value={contractFormData.successFeeRate ?? 3.5} onChange={(e) => handleContractFormChange('successFeeRate', Number(e.target.value))} className={editableInputClass} disabled={!contractFormData.customRate} />
                      ...
                      <div className="col-span-2 bg-purple-50 rounded-lg p-3 border border-purple-200">
                        <div className="text-sm font-medium text-purple-800">
                          合計: ¥{((contractFormData.initialFee ?? 30000) + (contractFormData.successFee || 0)).toLocaleString()}（初期費用 + 成功報酬）
                        </div>
                      </div>
                    </div>
                  )}
