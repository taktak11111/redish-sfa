...
- **[Mandatory for REDISH]**: Every requirement area must include at least one acceptance criteria starting with **"[画面表示]"** followed by a clear, natural Japanese description of the UI behavior (e.g., "[画面表示] 未認証ユーザーがアクセスした場合、『権限がありません』という警告が表示されること").
- **[Mandatory for REDISH]**: Before generating requirements, the AI must provide a high-level visual map (Mermaid or bullet list) of the project's overall features and highlight where the current specification fits in.